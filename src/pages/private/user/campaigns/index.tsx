// frontend/src/pages/private/user/campaigns/index.tsx
...
function CampaignsPage() {
  ...
  const getData = async () => {
    try {
      ...
      const response = await axios.get("/api/campaigns/get-by-user");
  ...
  const exportData = () => {
    const csvContent = [
      ['Name', 'Target Amount', 'Collected Amount', 'Progress %', 'Status', 'Created Date'],
    ...
    a.href = url;
    a.download = `my-campaigns-${dayjs().format('YYYY-MM-DD')}.csv`;
    a.click();
        ...
        <Space>
          ...
          <Tooltip title="Edit Campaign">
            <Button 
              type="text" 
              icon={<EditOutlined />}
              onClick={() => navigate(`/user/campaigns/edit/${record._id}`)}
              style={{ color: '#f59e0b' }}
            />
          </Tooltip>
